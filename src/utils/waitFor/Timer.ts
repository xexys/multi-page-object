/**
 * @file Копия реализации таймера для команды waitUntil из библиотеки WebdriverIO.
 *
 * @see https://github.com/webdriverio/webdriverio/blob/v8.6.7/packages/webdriverio/src/commands/browser/waitUntil.ts
 * @see https://github.com/webdriverio/webdriverio/blob/v8.6.7/packages/webdriverio/src/utils/Timer.ts
 *
 * Изменения после копирования:
 *  - В логике работы класса ничего не менял.
 *  - Заменил дефолтный экспорт класса на именованный.
 *  - Добавил экспорт для константы TIMEOUT_ERROR.
 *  - Добавил 'return undefined' где требовалось, чтобы успокоить eslint.
 *  - Заменил тип Function на Callable, чтобы успокоить eslint.
 *  - Исправил проверку, что результат thenable, так как падало для undefined и null значений.
 *    @see https://github.com/webdriverio/webdriverio/blob/v8.6.7/packages/webdriverio/src/utils/Timer.ts#L78
 */
...
export class Timer {
    ...
    private _resolve: Callable = NOOP;
    private _reject: Callable = NOOP;
    ...
    constructor(private _delay: number, private _timeout: number, private _fn: Callable, private _leading = false) {
